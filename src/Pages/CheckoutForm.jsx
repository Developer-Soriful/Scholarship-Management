...
const onPhotoChange = async (imgFile) => {
  // এখানে শুধু ফাইল নেবে
  if (!imgFile) return;
...
const createPaymentIntent = async (amount) => {
  const res = await axios.post("https://server-side-12-1x95.onrender.com/create-payment-intent", {
    amount,
  });
  return res.data;
};
...
const CheckoutForm = () => {
  ...
  const applyMutation = useMutation({
    mutationFn: ({ data, scholarship, user }) =>
      applyScholarship({ data, scholarship, user }),
    onSuccess: (data) => {
      setShowModal(false);
      reset();
  ...
  const handleSubmit = async (event) => {
    ...
    if (confirmError) {
      ...
      try {
        await axiosSecure.patch(`/scholarship/${id}`, {
          paymentStatus: "paid",
          paidBy: user.email,
        });
      } catch (err) {
      ...
      <div className="w-full max-w-md bg-white rounded-3xl shadow-xl p-8 md:p-10 border border-white/20">
        <div className="flex flex-col items-center mb-8">
          ...
          <div className="text-lg font-bold text-gray-900 mt-2">
            Amount: <span className="text-blue-600">${amount}</span> USD
          </div>
        </div>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-xl bg-black/20 bg-opacity-40">
          <div className="relative w-full max-w-lg mx-2 bg-white rounded-2xl shadow-2xl p-6 md:p-8 overflow-y-auto max-h-[90vh]">
            ...
            <form
              className="space-y-4"
              onSubmit={rhfHandleSubmit(onModalFormSubmit)}
            >
              <div>
              ...
              <div>
                ...
                <label className="flex items-center justify-center w-full px-4 py-2 bg-white border-2 border-dashed border-blue-400 rounded-lg cursor-pointer hover:bg-blue-50 transition">
                  ...
                  <span className="text-blue-600 font-semibold">
                    {watchedPhoto
                      ? watchedPhoto.name || "Photo Selected"
                      : "Upload Photo"}
                  </span>
                  <input
                    type="file"
